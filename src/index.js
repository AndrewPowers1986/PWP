...
const validation = [
    ...
    check('message', 'A message of 200 words or less is required.').trim().escape().isLength({min: 1, max: 2000})
]
...
const handlePostRequest = (req, res) => {
    ...
    mg.messages.create(process.env.MAILGUN_DOMAIN, mailgunData)
        .then(msg =>
            res.send(
                `<div class='alert alert-success' role='alert'>'E-mail successfully sent.'</div>`
            )
        )
        .catch(err =>
            res.send(
